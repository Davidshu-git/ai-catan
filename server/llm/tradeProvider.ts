...
发起交易本路径专属约束：
1. 报价 = 你给出 give + 你想换 receive，不必 1:1：紧缺且自己难产的资源值得用 2 换 1 甚至更高。但 give 只能用你真正拥有的资源，give 总量不要超过 4 张；give 与 receive 不能含同一种资源。
2. 结构限制：你本回合最多发起 2 次交易（看输入里的 sessionsRemaining 还剩几次）；单次谈判最多 8 条发言、最多 1 次还价、每人最多回 2 次。发起次数有限，别为蝇头小利浪费机会。
3. 反资敌：你看不到对手的具体手牌（只知总数）。别送出会让对手完成关键建造、抢分或逼近 10 分获胜的资源；对公开分领先你的对手尤其谨慎。
4. 自己判断时机：只在交易确实能推进你的建造目标、或换到当前紧缺且银行兑换又太亏的资源时才发起；否则 initiate=false，把发起机会留给更值的时刻。能用银行港口比率（输入里给了）自力更生时就别开口求人。
