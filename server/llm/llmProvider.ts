...
/**
 * 卡坦核心规则块（决策 / 交易 / 社交 3 路 LLM 调用共享）。
 * 抽出来是为了让每条 LLM 调用都"全程明白"完整规则，避免交易/社交路径
 * 因为只看简化版规则而误判（如把砖港当资源来源、误以为对手回合也能赢等）。
 * 注意：这块只含"客观规则"，不含任何路径专属的输出格式 / 角色身份 / 铁律。
 * 路径专属约束（如决策必须挑 actionId、交易必须输出 JSON 报价）由各 provider
 * 自己的 system prompt 补在本块前/后。
 */
export const CATAN_RULES_BLOCK = `游戏规则（核心，整局通用）：
- 胜利条件：总分（self.totalVP，含隐藏的胜利点卡）达到 ≥10 分**且必须在你自己的回合**才能宣告胜利（轮到你掷骰前 / 主回合都可以）；对手回合即使你被动加分（被换卡/被偷）也不会立即结算。你只看得到自己的 totalVP；对手仅暴露 publicVP（不含其隐藏胜利点卡），真实分可能更高。
- 计分：房屋=1 分，城市=2 分，最长路=2 分，最大军队=2 分，每张胜利点发展卡=1 分（隐藏）。
- 最长路：拥有 ≥5 段连续道路（被对方**房屋或城市**插在路网中间的顶点上才算截断，对方的路不会截断你；自己的路也算），且严格长于当前持有者才能抢到这 2 分；后来者如果只是追平、不严格更长则持有者不变；持有者被截断到 <5 段或被反超后，2 分立刻转移。
- 最大军队：累计打出 ≥3 张骑士，且严格多于当前持有者，才抢到这 2 分。
- 资源产出：每回合掷两颗骰子，点数之和等于某地块数字（2-6 或 8-12，不含 7）时，与该地块相邻的房屋各产 1 张、城市各产 2 张对应资源；强盗所在地块本回合不产出。**掷出 7 时所有地块都不产出资源**，改触发"弃半 + 移强盗 + 偷牌"流程。
- 银行资源短缺规则：每种资源银行共 19 张。某回合若**该资源银行剩余张数不够分配**且**有 >1 个玩家需要它**，则本回合**所有人**都拿不到该资源（公平规则，避免分配纠纷）；若只有 1 人需要则照常发。
- 发展卡效果：骑士=把强盗移到一个**不是当前所在**的地块，然后从"**在该新地块上有房屋或城市**"的对手里选 1 人偷 1 张随机手牌（不是"邻座玩家"！没人有建筑则只移强盗、不偷牌；计入最大军队）；修路=立刻免费建 2 条路（仍须遵守连通规则，资源不消耗）；丰收=从银行任取 2 张资源（**可以同 2 张或两种各 1 张，任意组合**）；垄断=指定 1 种资源，所有对手把手里该资源全交给你（对手没有就不交）；胜利点=立即 +1 分且隐藏（不进手牌、不占"每回合限打 1 张"名额）。每回合最多打 1 张发展卡（roll 阶段或 main 阶段都可，但合计 1 次）；当回合新购的发展卡下回合才能用（胜利点卡例外，立即生效）。
- 建造上限：房屋最多 5、城市最多 4、道路最多 15；**升城会把那座房屋退回 supply**（即升完城后你的房屋数 -1、城市数 +1），所以"已有 5 房 4 城"仍可继续升城（升一个房 → 4 房 5 城，但 4 城已到上限故再升不能）。
- 建造连通规则：①道路必须接在你已有的道路、房屋或城市旁，不能凭空建在路网之外；对手的房屋/城市会截断路网、无法穿过它继续延伸。②主回合建房屋除满足距离规则外，还必须建在与你自己道路相邻的顶点上（setup 开局摆放例外，不需连路）。③城市只能把你自己已有的房屋原地升级，不能凭空新建。④距离规则：任何房屋或城市的"相邻顶点"（= 通过 1 条边直接相连，距离 = 1）都不能再建任何玩家的房屋（两建筑物之间须至少隔 1 个顶点）。
- 交易比率（**港口只是兑换折扣，不会产生任何资源**）：与银行默认 4:1（4 张同种资源换 1 张任意资源）；占领"通用"港口顶点降到 3:1（3 同换 1 任意）；占领某资源专属港口（如砖港、麦港）该资源降到 2:1（**2 张该资源换 1 张任意资源**）。注意：占领砖港**不是**白拿砖、也不是别的资源 2:1 换砖；它只是允许你"用 2 砖换任意 1 张资源"。
- 开局摆放（setup）：每人先后放 2 个房屋各带 1 条路，蛇形顺序（P0→P1→P2→P3 放第一屋+路，然后 P3→P2→P1→P0 放第二屋+路）。**放第一个房屋（setup1）不发任何资源**；放**第二个房屋（setup2）**时立刻从相邻每个非沙漠地块各领 1 张资源（沙漠不产）。setup 阶段建路不消耗资源、也不需要先有路（可贴在刚放的那座房屋旁），但放完路才能轮到下家。
- 玩家间交易：在自己主回合可以与其他玩家**任意比例**交换资源（不必 1:1，不能 0 换正；不能换分数、地块或发展卡）；交易由双方同意才成交，对方可以拒绝或还价。
- 信息隐藏：你看不到对手的具体手牌种类和发展卡类别，只能看到他们的手牌总数（handSize）和发展卡总数（devCardCount）。别假装已知对手底牌。
- 7 点弃半：掷出 7 时所有手牌数 >7 的玩家必须弃掉 floor(手牌数/2) 张，然后当前玩家移动强盗并偷牌；打骑士卡只移动强盗/偷牌，不触发弃半。`;
...
6. 建房/初始放房屋必须遵守距离规则：任何房屋或城市的"相邻顶点"（= 通过 1 条边直接相连的 2-3 个顶点，距离 = 1）都不能再建任何玩家的房屋。即两个建筑物之间必须**至少隔 1 个顶点**（≥2 条边距离）。legalActions 里已经过滤掉违规顶点
