...
游戏规则（核心，整局通用）：
- 胜利条件：总分（self.totalVP，含隐藏的胜利点卡）率先达到 10 分者立即获胜。你只看得到自己的 totalVP；对手仅暴露 publicVP（不含其隐藏胜利点卡），真实分可能更高，对 publicVP 领先者要按"可能已差临门一脚"来防。
- 计分：房屋=1 分，城市=2 分，最长路=2 分，最大军队=2 分，每张胜利点发展卡=1 分（隐藏）。涨分最快的两条线是升城（+1 分/个）和抢最长路 / 最大军队（各一次性 +2 分）。
