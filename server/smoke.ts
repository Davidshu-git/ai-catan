...
// SMOKE_DRIVE=1 时驱动 P0 人类完成首手 setup1，观察后续 AI 的 ai_thought 流。
//
// 用法（容器内，需 stack 在跑）：
...
socket.on('sync_state', (g: FullGame) => {
  ...
  // SMOKE_DRIVE=1：在 P0 的首手放一个 settlement + 一条相邻路
  if (DRIVE && !driven && g.state.phase === 'setup1' && g.state.current === 0 && g.state.setupStep === 'settlement') {
  ...
  if (DRIVE && g.state.phase === 'setup1' && g.state.current === 0 && g.state.setupStep === 'road') {
...
socket.on('ai_thought', (ev) => {
  ...
  if (thoughtCount >= 2 || (!DRIVE && thoughtCount >= 1)) {
...
setTimeout(() => {
  if (DRIVE) {
    console.log('→ 发送 new_game，准备代 P0 走首手');
...
if (!DRIVE) {
  setTimeout(() => {
    console.log(`ℹ️  收尾：sync=${syncCount} thought=${thoughtCount}（首手是人类，AI 在等）`);
    clearTimeout(timer);
    socket.disconnect();
    process.exit(0);
  }, 8000);
